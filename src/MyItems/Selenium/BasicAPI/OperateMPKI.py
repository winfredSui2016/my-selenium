...
class OperateMPKI(unittest.TestCase): #13
    ...
    def testOperateDialog(self): 
        ...
        driver.find_element_by_css_selector('input[type=text]').send_keys('xxxx') #The username box which showed on a new dialog below on 'tang-content'
        time.sleep(2)

        driver.find_element_by_css_selector('input[type=password]').send_keys('xxxx') #The username box which showed on a new dialog below on 'tang-content'
        time.sleep(2)
